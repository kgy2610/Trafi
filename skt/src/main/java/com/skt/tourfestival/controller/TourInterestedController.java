...
/**
 * Servlet implementation class TourInterestedController
 */
public class TourInterestedController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public TourInterestedController() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int result = tfService.tourInterested(map);

		response.setContentType("application/json; charset=UTF-8");
		response.getWriter().print(result);
		request.getRequestDispatcher("detail.tf?tno=" + tourNo).forward(request, response);
	}
